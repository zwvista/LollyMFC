...
void CUGGrid::OnMouseHover(UINT nFlags, CPoint point)
{
	int col;
	long row;
	...
	if(GetCapture() != this && (m_GI->m_hoverRow != row || m_GI->m_hoverCol != col))
	{
		if(m_GI->m_hoverCol >= 0 && m_GI->m_hoverRow >= 0)
			m_ctrl->GetCellType(m_GI->m_hoverCol, m_GI->m_hoverRow)->OnMouseLeave(m_GI->m_hoverCol, m_GI->m_hoverRow);
		m_GI->m_hoverRow = row;
		m_GI->m_hoverCol = col;
	}

	m_ctrl->GetCellType(m_GI->m_hoverCol, m_GI->m_hoverRow)->OnMouseHover(m_GI->m_hoverCol, m_GI->m_hoverRow, &point, nFlags);
	CUGCtrl::TrackMouseEvent(m_hWnd);
}

void CUGGrid::OnMouseLeave()
{
	if(m_GI->m_hoverCol >= 0 && m_GI->m_hoverRow >= 0){
		m_ctrl->GetCellType(m_GI->m_hoverCol, m_GI->m_hoverRow)->OnMouseLeave(m_GI->m_hoverCol, m_GI->m_hoverRow);
		m_GI->m_hoverRow = -1;
