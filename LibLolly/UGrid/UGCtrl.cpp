...
void CUGCtrl::OnSysColorChange() 
{
	...
	m_GI->m_cnrDefaults->SetBackColor( GetSysColor( COLOR_BTNFACE ) );
	m_GI->m_sideHdgDefaults->SetBackColor( GetSysColor( COLOR_BTNFACE ) );
	m_GI->m_topHdgDefaults->SetBackColor( GetSysColor( COLOR_BTNFACE ) );
...
BOOL CUGCtrl::AttachGrid(CWnd * wnd,UINT ID){
	...
	if( SubclassDlgItem(ID,wnd))
	{
		...
		if(wnd!= NULL)
		{
			...
			CUGCell cell;
			GetCornerDefault( &cell );
			cell.SetFont( GetFont( nIndex ) );
			SetCornerDefault( &cell );
			GetTopHeadingDefault( &cell );
			cell.SetFont( GetFont( nIndex ) );
			SetTopHeadingDefault( &cell );
			GetSideHeadingDefault( &cell );
			cell.SetFont( GetFont( nIndex ) );
...
//				m_editCell.SetPropertyFlags( m_editCell.GetPropertyFlags()|UGCELL_DONOT_LOCALIZE );
			...
			m_editCtrl->SetWindowPos(&wndTop,rect.left+4,rect.top+1,
				rect.right-rect.left-4,rect.bottom-rect.top-1,0);
...
int	CUGCtrl::GetCellIndirect(int col,long row,CUGCell *cell){
	...
	else
	{
		if (row < 0 && col < 0)
		{
			cell->CopyInfoFrom(m_GI->m_cnrDefaults);
			cell->SetXPStyle(XPCellTypeBorder);
		}
		else if (row < 0 && col >= 0)
		{
			cell->CopyInfoFrom(m_GI->m_topHdgDefaults);
			cell->SetXPStyle(XPCellTypeTopCol);
		}
		else if (col < 0 && row >= 0)
		{
			cell->CopyInfoFrom(m_GI->m_sideHdgDefaults);
...
/***************************************************
SetCornerDefault
	copies the given cell properties to the corner
	defaults. corner defaults are used when top
	corner buttons are being drawn. These values are applied
	first, then any properties set for the corner
  ...
  return
		...
		UG_ERROR	fail
****************************************************/
int	CUGCtrl::SetCornerDefault(CUGCell *cell){
	m_GI->m_cnrDefaults->CopyInfoFrom(cell);
	return UG_SUCCESS;
}

/***************************************************
GetCornerDefault
	Fills the given cell with the top corner default
	properties
  ...
  return
		...
		UG_ERROR	fail
****************************************************/
int	CUGCtrl::GetCornerDefault(CUGCell *cell){
	cell->CopyInfoFrom(m_GI->m_cnrDefaults);
	return UG_SUCCESS;
}

/***************************************************
SetTopHeadingDefault
copies the given cell properties to the top heading
defaults. Top heading defaults are used when a
top headings are being drawn. These values are applied
...
UG_ERROR	fail
****************************************************/
int	CUGCtrl::SetTopHeadingDefault(CUGCell *cell){
	m_GI->m_topHdgDefaults->CopyInfoFrom(cell);
	return UG_SUCCESS;
}

/***************************************************
GetHeadingDefault
Fills the given cell with the top heading default
properties
...
UG_ERROR	fail
****************************************************/
int	CUGCtrl::GetTopHeadingDefault(CUGCell *cell){
	cell->CopyInfoFrom(m_GI->m_topHdgDefaults);
	return UG_SUCCESS;
}
...
/***************************************************
SetHeadingDefault
copies the given cell properties to the side heading
defaults. Side heading defaults are used when a
side headings are being drawn. These values are applied
first, then any properties set for the heading cell
being drawn are applied overtop
...
UG_ERROR	fail
****************************************************/
int	CUGCtrl::SetSideHeadingDefault(CUGCell *cell){
	m_GI->m_sideHdgDefaults->CopyInfoFrom(cell);
	return UG_SUCCESS;
}

/***************************************************
GetSideHeadingDefault
Fills the given cell with the side heading default
properties
...
UG_ERROR	fail
****************************************************/
int	CUGCtrl::GetSideHeadingDefault(CUGCell *cell){
	cell->CopyInfoFrom(m_GI->m_sideHdgDefaults);
	return UG_SUCCESS;
}
...
void CUGCtrl::TrackMouseEvent( HWND hwnd )
{
	TRACKMOUSEEVENT tme;
...
int CUGCtrl::OnInsertCol(int col)
{
	UNREFERENCED_PARAMETER(col);
	return UG_SUCCESS;
}

int CUGCtrl::OnAppendCol()
{
	return UG_SUCCESS;
}

int CUGCtrl::OnDeleteCol(int col)
{
	UNREFERENCED_PARAMETER(col);
	return UG_SUCCESS;
}

int CUGCtrl::OnInsertRow(long row)
{
	UNREFERENCED_PARAMETER(row);
	return UG_SUCCESS;
}

int CUGCtrl::OnAppendRow()
{
	return UG_SUCCESS;
}

int CUGCtrl::OnDeleteRow(long row)
{
	UNREFERENCED_PARAMETER(row);
	return UG_SUCCESS;
}
