...
void CUGSideHdg::OnMouseMove(UINT nFlags, CPoint point)
{
	...
	{
		if(m_GI->m_hoverRow == -1 && m_GI->m_hoverCol == -1)
...
void CUGSideHdg::OnMouseHover(UINT nFlags, CPoint point)
{
	int col;
	long row;
	RECT rect;
	...
	BOOL bCaptured = GetCapture() == this;

	if(m_GI->m_hoverRow == row && m_GI->m_hoverCol == col)
	{
		if(bCaptured && m_GI->m_hoverState != 2)
		{
			m_GI->m_hoverState = 2;
			Invalidate();
		}
		else if(!bCaptured && m_GI->m_hoverState != 0)
		{
			m_GI->m_hoverState = 0;
			Invalidate();
		}
