...
/*	if(overLapCol != col)
	...
	if(selected || (current && m_ctrl->m_GI->m_currentCellMode&2 && !m_ctrl->m_editInProgress))
	{
...
//		DrawBackground( dc, rect, backcolor, row, col, cell, (current != 0), (selected != 0));
		...
		{
			dc->ExtTextOut(left,orgTop,ETO_CLIPPED,rect,string,stringLen+1,NULL);
		}
	}
...
void CUGCellType::DrawBackground(CDC *dc,RECT *rect,COLORREF backcolor, int row, int col, CUGCell * cell, bool current, bool selected)
{
	...
	if(row < 0 && col >= 0)
	{
		HANDLE hTheme = ::OpenThemeData(NULL, L"HEADER");
		int iPartId = HP_HEADERITEM;
		int iStateId = row == m_ctrl->m_GI->m_hoverRow && col == m_ctrl->m_GI->m_hoverCol ?
			m_ctrl->m_GI->m_hoverState == 2 ? HIS_PRESSED :
			m_ctrl->m_GI->m_hoverState == 0 ? HIS_HOT :
			HIS_NORMAL : HIS_NORMAL;
		rect->left -= 1;
		rect->right += 1;
		rect->top -= 1;
	...
	else if(row >= 0 && col < 0)
	{
		...
		rect->bottom += 1;
		CRect rc1 = *rect;
		CRect rc2(0, 0, rc1.Height(), rc1.Width());
		CDC memDC;
		memDC.CreateCompatibleDC(dc);
		CBitmap bmp;
		bmp.CreateCompatibleBitmap(dc, rc2.Width(), rc2.Height());
		CBitmap* pOldBmp = memDC.SelectObject(&bmp);
		::DrawThemeBackground(hTheme, memDC, iPartId, iStateId, rc2, NULL);
		POINT p[3] = 
		{
			{rc1.left, rc1.top}, 
			{rc1.left, rc1.bottom}, 
		...
		::PlgBlt(*dc, p, memDC, 0, 0, rc2.Width(), rc2.Height(), NULL, 0, 0);
		::CloseThemeData(hTheme);
...
void CUGCellType::DrawBorder(CDC *dc,RECT *rect,RECT *rectout,CUGCell * cell)
{
	...
	//if (!m_useThemes || cell->GetXPStyle() != XPCellTypeTopCol && cell->GetXPStyle() != XPCellTypeLeftCol)
	{
...
/***************************************************
OnMouseHover
This function is called when the mouse  is hovering over
a cell of this celltype.
Params:
col - column that the mouse is over
row - row that the mouse is over
point - point where the mouse is
flags - mouse move flags (see WM_MOUSEHOVER)
Return:
TRUE - if the event was processed
FALSE - if the event was not
****************************************************/
BOOL CUGCellType::OnMouseHover(int col,long row,POINT *point,UINT flags)
{
	UNREFERENCED_PARAMETER(col);
	UNREFERENCED_PARAMETER(row);
	UNREFERENCED_PARAMETER(*point);
...
/***************************************************
OnMouseLeaving
This function is called when the mouse  is leaving
a cell of this celltype.
Params:
col - column that the mouse is over
row - row that the mouse is over
...
FALSE - if the event was not
****************************************************/
BOOL CUGCellType::OnMouseLeave(int col,long row)
{
	UNREFERENCED_PARAMETER(col);
	UNREFERENCED_PARAMETER(row);
	return FALSE;
