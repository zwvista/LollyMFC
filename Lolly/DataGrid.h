// MyCUG.h : interface of the CDataGrid class
//
/////////////////////////////////////////////////////////////////////////////
#pragma once
...
class CDataGrid :public CUGCtrl
{
public:
	CDataGrid();
	~CDataGrid();
...
protected:
	...
	BOOL m_bAllowAddNew;
	BOOL m_bColsResized;
	CADORecordset2* m_pRS;
	SDataGridColumnInfo* m_ci;
	int m_nNumCols;
	int m_nRowEditing;
	vector<CString> m_vstrEditing;
	int m_nIndexCol;
	CString m_strSQL;
	int m_nInitialSortCol;
	CUGRecordMarkerCellType m_rmCellType;
	CPen m_penGrid;

	bool IsAddNewRow(long row) {return m_bAllowAddNew && row == GetNumberRows() - 1;}
...
public:
	boost::signals2::signal<void(int, long, CUGCell*)> m_sigCellFormatting;

	virtual void BindData(CADORecordset2* pRS, LPCTSTR pszSQL, SDataGridColumnInfo* ci);
	void SetIndex(LPCTSTR pszIndex);
	void ResizeCols();
	void Init(int colSort);
	void Rebind();
	LPCTSTR	QuickGetText(int col,long row);

	void SetAllowDelete(BOOL val) {m_bAllowDelete = val;}
	void SetAllowAddNew(BOOL val) {m_bAllowAddNew = val;}
	void SetInitialSortCol(int col) {m_nInitialSortCol = col;}
	BOOL GetAllowDelete() const {return m_bAllowDelete;}
	BOOL GetAllowAddNew() const {return m_bAllowAddNew;}
	int GetInitialSortCol() const {return m_nInitialSortCol;}
	LPCTSTR GetColName(int col) const {return m_ci[col].pszFieldName;}
	LPCTSTR GetColCaption(int col) const {return m_ci[col].pszCaption;}
	BOOL IsInitializing() const {return m_nRowEditing == -2; }
	BOOL IsCurrentRowAddNewRow() {return IsAddNewRow(GetCurrentRow());}
	...
	virtual int OnInsertCol(int col);
	virtual int OnAppendCol();
	virtual int OnDeleteCol(int col);
	virtual int OnInsertRow(long row);
	virtual int OnAppendRow();
	virtual int OnDeleteRow(long row);
